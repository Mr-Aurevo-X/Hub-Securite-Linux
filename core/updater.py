...
GEST_REPO = "Mr-Aurevo-X/Hub-Securite-Linux"
...
FLATPAK_ASSET = "org.mraurevox.HubSecurite.flatpak"
...
APP_ID = "org.mraurevox.HubSecurite"
TAG_PREFIX = "v"
...
def local_version() -> str:
    ...
    if host.is_flatpak():
        candidates.append(Path("/app/share/hub-securite/VERSION"))
    candidates.append(Path(__file__).resolve().parent.parent / "VERSION")
    if not host.is_flatpak():
        candidates.append(Path.home() / ".local" / "share" / "hub-securite" / "VERSION")
    seen: set[Path] = set()
...
def updates_dir() -> Path:
    ...
    path = Path.home() / ".local" / "share" / "hub-securite" / "updates"
...
def format_update_dialog_body(info: dict[str, Any]) -> str:
    ...
    parts = [
        f"Hub Sécurité {latest} disponible (vous avez {current}).",
        "",
...
def app_display_name() -> str:
    return f"Hub Sécurité {local_version()}"


def _user_agent() -> str:
    return f"HubSecurite/{local_version()}"
...
def install_native_tarball(path: Path) -> None:
    ...
    # ~/.local/share/hub-securite would delete a nested source mid-copy.
...
def write_check_report_script(
    *,
    ...
    lines = [
        ...
        "set -euo pipefail",
        'echo "=========================================="',
        'echo " Hub Sécurité — vérification MAJ"',
        'echo "=========================================="',
        "echo",
...
def write_update_script(info: dict[str, Any]) -> Path:
    ...
    # /tmp — never extract under ~/.local/share/hub-securite (rsync self-delete).
...
set -euo pipefail
echo "=========================================="
echo " Hub Sécurité — mise à jour Flatpak {version}"
echo "=========================================="
echo
echo "==> Téléchargement…"
{download}
...
sleep 1
echo
echo "OK — Hub Sécurité {version} installé."
...
set -euo pipefail
echo "=========================================="
echo " Hub Sécurité — mise à jour native {version}"
echo "=========================================="
echo
echo "==> Téléchargement…"
{download}
...
echo
echo "==> Relance de l'application…"
if command -v hub-securite >/dev/null 2>&1; then
  nohup hub-securite >/dev/null 2>&1 &
elif [[ -x "$HOME/.local/bin/hub-securite" ]]; then
  nohup "$HOME/.local/bin/hub-securite" >/dev/null 2>&1 &
elif [[ -f "$HOME/.local/share/hub-securite/LANCER.sh" ]]; then
  nohup bash "$HOME/.local/share/hub-securite/LANCER.sh" >/dev/null 2>&1 &
else
  echo "ATTENTION : lanceur introuvable — relancez manuellement hub-securite."
fi
sleep 1
echo
echo "OK — Hub Sécurité {version} installé."
