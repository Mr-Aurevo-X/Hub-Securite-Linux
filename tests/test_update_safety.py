...
def _release_item(version: str) -> dict:
    return {
        ...
        "draft": False,
        "prerelease": False,
        "html_url": f"https://github.com/Mr-Aurevo-X/Hub-Securite-Linux/releases/tag/v{version}",
        "body": "notes",
        "assets": [
            {
                "name": "org.mraurevox.HubSecurite.flatpak",
                "browser_download_url": (
                    "https://github.com/Mr-Aurevo-X/Hub-Securite-Linux/releases/download/"
                    f"v{version}/org.mraurevox.HubSecurite.flatpak"
                ),
            }
        ],
    }
...
def test_update_channel_is_flatpak_only() -> None:
    ...
    assert url.endswith("org.mraurevox.HubSecurite.flatpak")
    assert "Hub-Securite-Linux/releases/download/v" in url
...
def test_flatpak_install_block_includes_menu_shortcut() -> None:
    ...
    assert "rm -f org.mraurevox.HubSecurite.flatpak" in block
    assert "wget --no-continue -O org.mraurevox.HubSecurite.flatpak" in block
...
def test_update_urls_allow_our_github_only() -> None:
    ...
    ok_flatpak = (
        "https://github.com/Mr-Aurevo-X/Hub-Securite-Linux/releases/download/"
        "v1.4.4/org.mraurevox.HubSecurite.flatpak"
    )
...
def test_plugin_rejects_path_escape_and_symlink(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # plugins_dir uses XDG_DATA_HOME/hub-securite/plugins
    real = tmp_path / "hub-securite" / "plugins"
