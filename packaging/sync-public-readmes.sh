...
put_legal_file() {
  local repo="$1"
  local dest="LEGAL-Hub-Securite.md"
  local b64 sha
  b64="$(base64 -w0 < "${ROOT}/LEGAL.md")"
  sha="$(gh api "repos/${repo}/contents/${dest}" --jq .sha 2>/dev/null || true)"
  local args=(-f message="docs: Hub Sécurité LEGAL.md (copyright / CGU / RGPD)" -f content="${b64}")
...
echo "Flatpak : https://github.com/Mr-Aurevo-X/Hub-Securite/releases"
