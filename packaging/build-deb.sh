#!/usr/bin/env bash
# Hub Sécurité - construit un .deb simple (Debian/Ubuntu)
# Installe sous /opt/hub-securite + /usr/bin/hub-securite + entree .desktop
...
cat > "${DEB_ROOT}/usr/bin/hub-securite" << 'WRAP'
#!/usr/bin/env bash
exec python3 /opt/hub-securite/main.py "$@"
WRAP
chmod 755 "${DEB_ROOT}/usr/bin/hub-securite"

cat > "${DEB_ROOT}/usr/share/applications/hub-securite.desktop" << 'DESK'
[Desktop Entry]
Type=Application
Version=1.0
Name=Hub Sécurité
Comment=Gestion systeme Linux (monitoring, services, nettoyeur)
Exec=/usr/bin/hub-securite
Path=/opt/hub-securite
...

{
  ...
  echo "Upstream-Name: Hub Sécurité"
  echo "Source: https://github.com/Mr-Aurevo-X/Hub-Securite"
  echo
  echo "Files: *"
