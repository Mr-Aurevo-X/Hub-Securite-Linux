#!/usr/bin/env bash
# Hub Sécurité — installation utilisateur (~/.local) + deps systeme (sudo)
# SPDX-License-Identifier: GPL-3.0-or-later
set -euo pipefail

APP_ID="hub-securite"
APP_NAME="Hub Sécurité"
...
flatpak_hint() {
  ...
  echo "  https://github.com/Mr-Aurevo-X/Hub-Securite/releases"
...
install_app_files() {
  ...
  if need_cmd rsync; then
    ...
    for item in main.py VERSION LICENSE COPYRIGHT LEGAL.md README.md requirements.txt \
                LANCER.sh INSTALLER-RACCOURCI.sh Hub-Securite.desktop \
...
main() {
  ...
  echo "Flatpak (toutes distros) : https://github.com/Mr-Aurevo-X/Hub-Securite/releases"
}
