...
1. Hub Sécurité est un utilitaire d’administration **locale** de votre machine Linux.
...
- Stockage local uniquement : `~/.config/hub-securite/` (préférences, `fleet.json`) et `~/.local/share/hub-securite/` (cache de mises à jour, plugins).
...
1. Hub Sécurité is a **local** Linux system-administration utility.
...
- Local storage only: `~/.config/hub-securite/` (preferences, `fleet.json`) and `~/.local/share/hub-securite/` (update cache, plugins).
